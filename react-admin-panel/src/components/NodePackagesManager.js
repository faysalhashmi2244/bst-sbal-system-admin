...
const NodePackagesManager = ({ account }) => {
  ...
  const fetchNodePackages = async () => {
    try {
      ...
      setLoading(true);
      const nodePackagesContract = await getNodePackagesContract(
        account.signer
      );
  ...
  const updateNodePackage = async () => {
    try {
      ...
      const durationInSeconds =
        parseInt(editPackage.durationInDays) * 24 * 60 * 60;
  ...
  const togglePackageStatus = async (packageId, currentStatus) => {
    try {
      ...
      const tx = await nodePackagesContract.setNodePackageActive(
        packageId,
        !currentStatus
      );
      ...
      toast({
        ...
        } package... Please wait for confirmation`,
        status: "info",
        duration: 5000,
        isClosable: true,
      });
                  ...
                  <Tr key={pkg.id}>
                    ...
                    <Td>
                      {Number(pkg.duration) == 0
                        ? "Lifetime"
                        : Math.floor(Number(pkg.duration) / (24 * 60 * 60))}
                    </Td>
                    ...
                    <Td>
                      <Switch
                        ...
                        onChange={() =>
                          togglePackageStatus(pkg.id, pkg.isActive)
                        }
                      />
                    </Td>
                  </Tr>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalBody>
            ...
            <FormControl mb={4}>
              <FormLabel>Booster Percentage (%)</FormLabel>
              <Input
                name="roiPercentage"
